function [performancevector, performancenames, T, yNa, Y] = efast3v2REUPstrainsV2b(X, kvector, kidx, tmax, odeoptions)
...
theta_yA      = kvector(13:22);
theta_yB      = kvector(23:32);
theta_yC      = kvector(33:42);
omega         = kvector(43:45);
...
%% ===== SIMULATE MODEL ===================================================
[T, Y] = ode15s( @(T, Z) ode3v2REUPstrains(T, Z, theta_biomass, theta_yA, theta_yB, theta_yC, omega), [0, tmax], Y0, odeoptions);
...
for t = 1:length(T)
    [dY_by_dt(t,:), J0_grow(t,:), J0_upt_gluc(t,:), J0_leak_yX_X(t,:), J0_upt_yX_Y(t,:)] = ode3v2REUPstrains(T(t), Y(t,:), theta_biomass, theta_yA, theta_yB, theta_yC, omega);
end
