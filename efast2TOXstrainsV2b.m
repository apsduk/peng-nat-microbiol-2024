function [performancevector, performancenames, T, yNa, Y] = efast2TOXstrainsV2b(X, kvector, kidx, tmax, odeoptions)
...
%% ===== SIMULATE MODEL ===================================================
[T, Y] = ode15s( @(T, Z) ode2TOXstrains(T, Z, theta_biomass, theta_yA, theta_yB, omega), [0, tmax], Y0, odeoptions);
...
for t = 1:length(T)
    [dY_by_dt(t,:), J0_grow(t,:), J0_upt_gluc(t,:), J0_leak_yX_X(t,:), J0_upt_yX_Y(t,:)] = ode2TOXstrains(T(t), Y(t,:), theta_biomass, theta_yA, theta_yB, omega);
end
...
performancevector( 9) = max(J0_upt_yX_Y(:,1));   performancenames{ 9} = 'max(J_upt_yB_A)';
performancevector(10) = max(J0_upt_yX_Y(:,2));   performancenames{10} = 'max(J_upt_yA_B)';
