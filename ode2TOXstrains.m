function [dZ_by_dt, J0_grow, J0_upt_gluc, J0_leak_yX_X, J0_upt_yX_Y] = ode2strains(T, Z, theta_biomass, theta_yA, theta_yB, omega)
...
% --- Toxicity ------------------------------------------------------------
Tox_yA = 1/(1 + (A/I_yA_A) + (B/I_yA_B));
Tox_yB = 1/(1 + (A/I_yB_A) + (B/I_yB_B));

% --- Specific growth rate ------------------------------------------------
J_grow_yA = min([gamma_B*J_upt_yA_B, gamma_gluc*(1 - phi_yA_A)*J_upt_yA_gluc]).*Tox_yA;
J_grow_yB = min([gamma_A*J_upt_yB_A, gamma_gluc*(1 - phi_yB_B)*J_upt_yB_gluc]).*Tox_yB;
