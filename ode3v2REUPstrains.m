function [dZ_by_dt, J0_grow, J0_upt_gluc, J0_leak_Y, J0_upt_Y] = ode3v2REUPstrains(T, Z, theta_biomass, theta_yA, theta_yB, theta_yC, omega)
...
Vmax_yA_A    = theta_yA(4); % Maximum A uptake rate by strain A
Km_yA_A      = theta_yA(5); % Michaelis constant for A uptake by strain A
Vmax_yA_B    = theta_yA(6); % Maximum B uptake rate by strain A
Km_yA_B      = theta_yA(7); % Michaelis constant for B uptake by strain A
Vmax_yA_C    = theta_yA(8); % Maximum C uptake rate by strain A
Km_yA_C      = theta_yA(9); % Michaelis constant for C uptake by strain A
delta_yA_A   = theta_yA(10); % Number of A molecules produced per molecule of glucose is consumed
...
Vmax_yB_B    = theta_yB(6); % Maximum B uptake rate by strain B
Km_yB_B      = theta_yB(7); % Michaelis constant for B uptake by strain B
Vmax_yB_C    = theta_yB(8); % Maximum C uptake rate by strain B
Km_yB_C      = theta_yB(9); % Michaelis constant for C uptake by strain B
delta_yB_B   = theta_yB(10); % Number of B molecules produced per molecule of glucose is consumed
...
Vmax_yC_C    = theta_yC(8); % Maximum C uptake rate by strain C
Km_yC_C      = theta_yC(9); % Michaelis constant for C uptake by strain C
delta_yC_C   = theta_yC(10); % Number of C molecules produced per molecule of glucose is consumed
...
J_upt_yC_C = (Vmax_yC_C*C)/(Km_yC_C + C);

% ----- Specific growth rate ----------------------------------------------
J_grow_yA_G = gamma_gluc*(1 - phi_yA_A)*J_upt_yA_gluc;
J_grow_yA_A = gamma_A*J_upt_yA_A;
...
J_grow_yB_B = gamma_B*J_upt_yB_B;
...
J_grow_yC_C = gamma_C*J_upt_yC_C;

% ----- Calculate specific growth rates -----------------------------------
J_grow_yA = min([J_grow_yA_G, J_grow_yA_A, J_grow_yA_B, J_grow_yA_C]);
J_grow_yB = min([J_grow_yB_G, J_grow_yB_A, J_grow_yB_B, J_grow_yB_C]);
J_grow_yC = min([J_grow_yC_G, J_grow_yC_A, J_grow_yC_B, J_grow_yC_C]);
...
dA_by_dt = J_leak_yA_A*yAa - J_upt_yA_A*yAa - J_upt_yB_A*yBa - J_upt_yC_A*yCa;

% --- Amino acid B dynamics -----------------------------------------------
dB_by_dt = J_leak_yB_B*yBa - J_upt_yA_B*yAa - J_upt_yB_B*yBa - J_upt_yC_B*yCa;
 
% --- Amino acid C dynamics -----------------------------------------------
dC_by_dt = J_leak_yC_C*yCa - J_upt_yA_C*yAa - J_upt_yB_C*yBa - J_upt_yC_C*yCa;
...
J0_upt_Y     = [J_upt_yA_A*yAa + J_upt_yB_A*yBa + J_upt_yC_A*yCa, J_upt_yA_B*yAa + J_upt_yB_B*yBa + J_upt_yC_B*yCa, J_upt_yA_C*yAa + J_upt_yB_C*yBa + J_upt_yC_C*yCa];
