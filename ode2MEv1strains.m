function [dZ_by_dt, J0_grow, J0_upt_gluc, J0_leak_yX_X, J0_upt_yX_Y, J0_meta] = ode2MEv1strains(T, Z, theta_biomass, theta_yA, theta_yB, theta_meta, omega)
...
Y    = Z(9); % Y concentration in the cutlure vessel
...
delta_yA_X   = theta_meta(1); 
phi_yA_X     = theta_meta(2);
Vmax_yB_XY   = theta_meta(3);
Km_yB_XY     = theta_meta(4);
...
% --- Pathway production rates --------------------------------------------
J_conv_yA_GX = J_upt_yA_gluc*delta_yA_X*phi_yA_X;
J_conv_yB_XY = (Vmax_yB_XY*X)/(Km_yB_XY + X);

% --- Specific growth rate ------------------------------------------------
J_grow_yA_G = gamma_gluc*(1 - phi_yA_A - phi_yA_X)*J_upt_yA_gluc;
...
J_grow_yB = min([J_grow_yB_G, J_grow_yB_A]);
...
dA = J_leak_yA_A*yAa - J_upt_yB_A*yBa;

% --- Amino acid B dynamics -----------------------------------------------
dB = J_leak_yB_B*yBa - J_upt_yA_B*yAa;

% --- Product from strain A -----------------------------------------------
dX = J_conv_yA_GX*yAa - J_conv_yB_XY*yBa;

% --- Product from strain B -----------------------------------------------
dY = J_conv_yB_XY*yBa;

%% === RETURN =============================================================
dZ_by_dt = [dGluc_by_dt; dyA_by_dt; dyB_by_dt; dyAa_by_dt; dyBa_by_dt; dA; dB; dX; dY];
...
J0_upt_yX_Y  = [J_upt_yB_A*yBa    , J_upt_yA_B*yAa];
J0_meta      = [J_conv_yA_GX*yAa  , J_conv_yB_XY*yBa];
