function [performancevector, performancenames, T, yNa, Y] = efast3v1astrainsV2(X, kvector, kidx, tmax, odeoptions)
...
theta_yA      = kvector(13:18);
theta_yB      = kvector(19:24);
theta_yC      = kvector(25:30);
omega         = kvector(31:33);
...
%% ===== SIMULATE MODEL ===================================================
[T, Y] = ode15s( @(T, Z) ode3v1astrains(T, Z, theta_biomass, theta_yA, theta_yB, theta_yC, omega), [0, tmax], Y0, odeoptions);
...
for t = 1:length(T)
    [dY_by_dt(t,:), J0_grow(t,:), J0_upt_gluc(t,:), J0_leak_yX_X(t,:), J0_upt_yX_Y(t,:)] = ode3v1astrains(T(t), Y(t,:), theta_biomass, theta_yA, theta_yB, theta_yC, omega);
end
