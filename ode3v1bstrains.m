function [dZ_by_dt, J0_grow, J0_upt_gluc, J0_leak_Y, J0_upt_Y] = ode3v1bstrains(T, Z, theta_biomass, theta_yA, theta_yB, theta_yC, omega)

%% ===== VARIABLES ========================================================
Z(Z < 0) = 0;
Gluc = Z( 1); % glucose concentration in the culture vessel
yA   = Z( 2); % A+/B- total population density of strain A
yB   = Z( 3); % B+/C- total population density of strain B
yC   = Z( 4); % C+/A- total population density of strain C
yAa  = Z( 5); % A+/B- total active population density of strain A
yBa  = Z( 6); % B+/C- total active population density of strain B
yCa  = Z( 7); % C+/A- total active population density of strain C
...
% ----- (e.g. lys production, ade uptake, his production [overflow]) --------
...
delta_yA_C   = theta_yA(7); % Number of C molecules produced per molecule of glucose is consumed
phi_yA_A     = omega( 1);   % Fraction of glucose uptake leaked in form of A production by strain A
phi_yA_C     = omega( 2);   % Fraction of glucose uptake leaked in form of C production by strain A

% ----- Strain B parameters (B+/C-) ---------------------------------------
% ----- (e.g. ade production, his uptake) ---------------------------------
...
Vmax_yB_C    = theta_yB(4); % Maximum A uptake rate by strain B
Km_yB_C      = theta_yB(5); % Michaelis constant for A uptake by strain B
delta_yB_B   = theta_yB(6); % Number of B molecules produced per molecule of glucose is consumed
phi_yB_B     = omega( 3);   % Fraction of glucose uptake leaked in form of B production by the strain B

% ----- Strain C parameters (C+/A-) ---------------------------------------
% ----- (e.g. his production, lys uptake) ---------------------------------
...
delta_yC_C   = theta_yC(6); % Number of C molecules produced per molecule of glucose is consumed
phi_yC_C     = omega( 4);   % Fraction of glucose uptake leaked in form of C production by the strain C
...
J_leak_yA_C = J_upt_yA_gluc*delta_yA_C*phi_yA_C;
...
J_grow_yA_G = gamma_gluc*(1 - phi_yA_A - phi_yA_C)*J_upt_yA_gluc;
...
J_grow_yA = min([J_grow_yA_G, J_grow_yA_B]);
J_grow_yB = min([J_grow_yB_G, J_grow_yB_C]);
J_grow_yC = min([J_grow_yC_A, J_grow_yC_G]);
...
dA_by_dt = J_leak_yA_A*yAa - J_upt_yC_A*yCa;

% --- Amino acid B dynamics -----------------------------------------------
dB_by_dt = J_leak_yB_B*yBa - J_upt_yA_B*yAa;

% --- Amino acid C dynamics -----------------------------------------------
dC_by_dt = J_leak_yC_C*yCa + J_leak_yA_C*yAa - J_upt_yB_C*yBa;
...
J0_upt_Y     = [J_upt_yC_A*yCa    , J_upt_yA_B*yAa    , J_upt_yB_C*yBa + J_leak_yA_C*yAa];
